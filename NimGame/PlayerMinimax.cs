...
namespace etf.nim.ma150129d
{
    public class PlayerMinimax : Player
    {
        ...
        public PlayerMinimax(State state, int maxDepth = 10)
        {
        ...
        { 
            this.tree = new Tree(state);
            tree.formTree(maxDepth);
            int val = minimax(tree.Root, maxDepth, 0);
            int i = 0;
        ...
        private int minimax(Tree.Node node, int maxDepth, int currDepth)
        {
            ...
            {
                currValue = minimax(node.Children[j], maxDepth, currDepth + 1);
